...
#define BUTTON_CONTENT_PADDING_LEFT 0
...
- (void)validateLayout
{
    ...
    if (self.items.count > 0) {
        ...
        for (NSUInteger i = 0; i < self.buttons.count; i++) {
            ...
            button.frame = CGRectMake(BUTTON_CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - BUTTON_CONTENT_PADDING_LEFT * 2, BUTTON_HEIGHT);
            ...
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(button.frame.origin.x, button.frame.origin.y, button.frame.size.width, 1.0f)];
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
    ...
    UIImage *highlightedImage = [self imageWithColor:[UIColor groupTableViewBackgroundColor]];
    switch (item.type) {
